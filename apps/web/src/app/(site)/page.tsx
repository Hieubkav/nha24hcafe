    ...
    return (
        <motion.section id="spaces" className="bg-zinc-950 py-24 sm:py-32" initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.2 }} variants={staggerContainer}>
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
                ...
                <motion.div variants={fadeInUp}><Tabs defaultValue={activeTab} onValueChange={setActiveTab} className="mt-16"><TabsList className="grid w-full grid-cols-1 sm:grid-cols-3 bg-zinc-900/80 border-zinc-800"><TabsTrigger value="study" className="data-[state=inactive]:text-neutral-400">Góc Học Tập</TabsTrigger><TabsTrigger value="meet" className="data-[state=inactive]:text-neutral-400">Phòng Họp</TabsTrigger><TabsTrigger value="relax" className="data-[state=inactive]:text-neutral-400">Góc Thư Giãn</TabsTrigger></TabsList><div className="mt-8 relative overflow-hidden"><AnimatePresence mode="wait"><motion.div key={activeTab} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3, ease: "easeInOut" }}>
